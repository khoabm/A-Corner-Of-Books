...
import acornerofbooks.utils.Constants;
...
public class LoginGoogleHandler extends HttpServlet {
    ...
    private static final String DETAIL = "Access/detail.jsp";
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            UserGoogleDTO user = getUserInfo(authenticationToken);
            System.out.println(user);

            UserManager manager = new UserManager();
            AccountDTO acc = manager.checkLogin(user.getEmail(), null);

            HttpSession ss = request.getSession();

            boolean check = true;
            CustomerDTO cus = new CustomerDTO();
            cus = manager.getCustomer(user.getEmail());

            if (acc == null) {
                check = manager.insertGoogleAccount(user.getEmail());
                acc = manager.checkLogin(user.getEmail(), null);
                ss.setAttribute("LOGIN_USER", acc);
                ss.setAttribute("user", user);
                response.sendRedirect(DETAIL);
            } else {
                if (null != acc.getPassword()) {                   
                    ss.setAttribute("ERROR_MSG", "User already exist");
            ...
            log("Error at GoogleLogin" + e.toString());
        }
