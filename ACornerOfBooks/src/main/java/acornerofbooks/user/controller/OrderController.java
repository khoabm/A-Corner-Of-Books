...
public class OrderController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            List<OrderDTO> list = dao.getOrder(email, currentPage);

            int totalMatched = dao.countMatched(email);
...
//            System.out.println(order.getTitles() + " " + order.getImages());
//        }
            request.setAttribute("listOrder", list);
            request.setAttribute("endPage", endPage);
            request.setAttribute("currentPage", currentPage);
            RequestDispatcher rd = request.getRequestDispatcher("/view/Customer/orderList.jsp");
            rd.forward(request, response);
        } catch (Exception e) {
            log("Error order site " + e.toString());
        }
    }
